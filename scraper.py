...
def collect_ads_balance():
    ...
    #     result = sheet.values().get(spreadsheetId="1-XxiPmxfN1QxnB93blEu6iY79qCYIh8w4YlSBHUzSFs", range="MediaBalance!A2:J2").execute()
    #     values = result.get("values", [])
    #     print(values)
    #     # Default value in case we don't find a match
    #     threshold_low = None
       

    #     # Loop through each row in the data
    #     responses = []  # List to collect the responses
    #     for row in values:
    #         if row[1] == "ADSTERRA":
    #             # sendRequestADS(row[3],row[4])
    #             response_data = sendRequestADS(row[3], row[4])  # Get the response data from the request

    #              # Fetch the second spreadsheet
    #             secondary_sheet = service.spreadsheets()
    #             secondary_result = secondary_sheet.values().get(
    #                 spreadsheetId="1PKdQR3zvTNwfDmlEwSayYaFfR7r97fZfRoUlQDUD2OE",
    #                 range="Sheet1!A2:ZZ"  # Adjust sheet name if needed
    #             ).execute()
    #             secondary_values = secondary_result.get("values", [])
    #             # Match the CODE (row[9]) from primary sheet with column A (index 0) of secondary sheet
    #             code_to_match = row[9]
                
    #             for secondary_row in secondary_values:
    #                 if len(secondary_row) > 4 and secondary_row[0] == code_to_match:
    #                     threshold_low = secondary_row[4]
    #                     break
    #             # print(threshold_low)
    #             responses.append({"brand":row[0],"platform":row[1],"balance":response_data.get('item',0.0),"currency":row[2], 'threshold_low':threshold_low, 'threshold_high':row[8]})
    #             print(responses)  # Optional, to separate each row output
    #         elif row[1] == "TRAFFICSTARS":
    #             response_data = sendRequestTRAFF(row[3], row[4])  # Get the response data from the request
    #             responses.append({"brand":row[0],"platform":row[1],"balance":response_data.get('balance', 0.0),"currency":row[2], 'threshold_low':row[7], 'threshold_high':row[8]})
    #             print(responses)  # Optional, to separate each row output
            
    #         elif row[1] == "PROFELLERADS":
    #             response_data = profellerAds(row[3], row[4])  # Get the response data from the request
    #             responses.append({"brand":row[0],"platform":row[1],"balance":response_data,"currency":row[2], 'threshold_low':row[7], 'threshold_high':row[8]})
    #             print(responses)  # Optional, to separate each row output
            
    #         elif row[1] == "ADXAD":
    #             response_data = ADXAD(row[3], row[5], row[6])  # Get the response data from the request
    #             balance = response_data.get("data", {}).get("balance", 0.0)
    #             responses.append({"brand":row[0],"platform":row[1],"balance":balance,"currency":row[2], 'threshold_low':row[7], 'threshold_high':row[8]})
    #             print(responses)  # Optional, to separate each row output
            
    #         elif row[1] == "CLICKADU":
    #             response_data = CLICKADU(row[3], row[4])  # Get the response data from the request
    #             balance = response_data.get("result", {}).get('advertiser', {}).get("balance", 0.0)
    #             responses.append({"brand":row[0],"platform":row[1],"balance":balance,"currency":row[2], 'threshold_low':row[7], 'threshold_high':row[8]})
    #             print(responses)  # Optional, to separate each row output

    #         elif row[1] == "EZMOB":
    #             response_data = EZMOB(row[3], row[4])  # Get the response data from the request
    ...
    #         elif row[1] == "POPADS":
    #             response_data = POPADS(row[3], row[4])  # Get the response data from the request
    #             balance = response_data["user"]["balance"]  # Extract balance from 'user' key
    #             # balance = response_data.get("result", {}).get('advertiser', {}).get("balance", 0.0)
    #             responses.append({"brand":row[0],"platform":row[1],"balance":balance,"currency":row[2], 'threshold_low':row[7], 'threshold_high':row[8]})
    #             print(responses)  # Optional, to separate each row output

    #         elif row[1] == "ADCASH":
    #             response_data = ADCASH(row[3],row[5],row[6])
    #             responses.append({"brand":row[0],"platform":row[1],"balance":response_data,"currency":row[2], 'threshold_low':row[7], 'threshold_high':row[8]})
    #             print(responses)  # Optional, to separate each row output

    #         elif row[1] == "TRAFFICNOMADS":
    #             response_data = TRAFFICNOMADS(row[3],row[4])
    #             balance = response_data.get("balance", 0)  # Default to 0 if key is missing
    #             responses.append({"brand":row[0],"platform":row[1],"balance":balance,"currency":row[2], 'threshold_low':row[7], 'threshold_high':row[8]})
    #             print(responses)  # Optional, to separate each row output

    #         elif row[1] == "RICHADS":
    #             response_data = RICHAD(row[3],row[5],row[6])
    #             responses.append({"brand":row[0],"platform":row[1],"balance":response_data,"currency":row[2], 'threshold_low':row[7], 'threshold_high':row[8]})
    #             print(responses)  # Optional, to separate each row output
    #         # still working on recatcha
    #         elif row[1] == "DAOAD":
    #             response_data = DAOAD(row[3],row[5],row[6])
    #             responses.append({"brand":row[0],"platform":row[1],"balance":response_data,"currency":row[2], 'threshold_low':row[7], 'threshold_high':row[8]})
    #             print(responses)  # Optional, to separate each row output

    #         elif row[1] == "ADPROFEX":
    #             response_data = ADPROFEX(row[3],row[4])
    #             responses.append({"brand":row[0],"platform":row[1],"balance":response_data.get("data", {}).get("balance", 0.0),"currency":row[2], 'threshold_low':row[7], 'threshold_high':row[8]})
    #             print(responses)  # Optional, to separate each row output
